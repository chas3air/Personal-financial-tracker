...
def count_code_metrics(directory):
    ...
    total_files = 0
    total_dirs = 0
    total_characters = 0  # Для подсчета общего количества символов
    file_counts_by_extension = defaultdict(int)
    char_counts_by_extension = defaultdict(int)
        ...
        for filename in filenames:
            if filename.endswith(('.go', '.sql', '.mod', '.sum', '.yml', '.yaml', 'Dockerfile', '.md')):
                ...
                with open(os.path.join(dirpath, filename), 'r', encoding='utf-8', errors='ignore') as file:
                    ...
                    total_characters += characters  # Обновление общего количества символов
                    char_counts_by_extension[extension] += characters

    return total_dirs, total_files, file_counts_by_extension, char_counts_by_extension, total_lines, total_characters

repo_path = '.'
dirs, files, file_counts, char_counts, lines_of_code, total_characters = count_code_metrics(repo_path)
...
print(f'Total characters across all files: {total_characters}')